...
byline: When writing a struct with the intention of it being reused, it’s important not to use boxed trait objects to represent interior data as they make it very hard to consume.
tags: [rust,beginner]
slug: dont-use-boxed-trait-objects-for-struct-internals
comments: true
---
...
To me, a trait describes a certain behaviour and should only provide methods that achieve that behaviour.
...
Now my requirements for the people has changed - I need to group them together in a `PeopleZoo`.
...
It's better to instead leverage Rust's type system and use traits as a way to describe the form of the data that's provided, but let the user provide whatever data they like (provided it fits within the traits constraints).
...
To demonstrate this, I'll first create another struct implementing `Person`.
...
Now I'm not trying to say that using boxed trait objects is bad, there are some cases where it makes a lot of sense.
What I am saying, however, is that using `Box<dyn Trait>` gives other developers that may consume your struct a hard time as there's not much flexibility.

You simply shouldn't force it upon anyone.

However, you shouldn't make it hard to use them either.
...
This can be done by calling `as_ref` or `as_mut` on the `Box` and calling the relevant method.
